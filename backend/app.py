...
def load_gtfs_static():
    ...
    # Trip headsigns + shape_id → route_id mapping
    trip_headsigns = {}
    shape_to_route = {}
    rail_shape_ids = set()
    with zf.open("trips.txt") as f:
        for row in csv.DictReader(io.TextIOWrapper(f, encoding="utf-8-sig")):
            ...
            if sid:
                ...
                shape_to_route.setdefault(sid, rid)
            if tid and hs:
    ...
    # Track shapes — deduplicate keeping shortest per start/end pair
    shape_points = {}
    ...
    for sid, pts in shape_points.items():
        ...
        colour = line_colour(rid)
        all_shapes.append({"coords": coords, "route_id": rid, "colour": colour, "shape_len": len(coords)})

    # Keep shortest shape per start/end pair so branch colours show over trunk
    sig_map = {}
    for s in all_shapes:
        c   = s["coords"]
        sig = min(
        ...
        if sig not in sig_map or s["shape_len"] < sig_map[sig]["shape_len"]:
            sig_map[sig] = s

    shapes = sorted(sig_map.values(), key=lambda x: -x["shape_len"])
